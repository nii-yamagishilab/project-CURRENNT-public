...
sub gen_wave($$$$$$) {
   ...
   foreach $file (@FILE) {
	  ...
	  if ($useMLPG){
	     ...
	     for ( $i = 1 ; $i <= $T ; $i++ ) {
		 shell_mute("cat $nndatdir/var/mgc.var >> $gendir/$base.mgc_delta.var");
	     }
	     ...
	     $spgendir = "$gendir";
	      ...
	      print "No MLPG on MGC\n";
	     # just rename the .mgc_delta as .mgc
	     shell("mv $spgendir/$base.mgc_delta $gendir/$base.mgc");
	     $spgendir = "$gendir";
         }
      }
      
      if ($synWav && -s "$gendir/$base.mgc" ) {
	  if ( $useSigPF && $useSigPF2 > 0.001 ) {
	      shell( "$MCPF -a $fw -m " . ( $ordr{'mgc'} - 1 ) . " -b $useSigPF2 -l $il $gendir/$base.mgc > $gendir/$base.p_mgc" );
	      $mgc = "$gendir/$base.p_mgc";
	      print "Post-filter on MGC\n";
	  }
	  else {
	      print "No Post-filter on MGC\n";
	      $mgc = "$gendir/$base.mgc";
	  }

         if ( $gm == 0 ) {
            shell( "$MGC2SP -a $fw -g $gm -m " . ( $ordr{'mgc'} - 1 ) . " -l $fp -o 2 $mgc | $SOPR -d 32768 -P  > $gendir/$base.sp" );
         }
         else {
	  ...
	  if ($useMLPG){
	     ...
	     for ( $i = 1 ; $i <= $T ; $i++ ) {
		 shell_mute("cat $nndatdir/var/iplf0.var >> $gendir/$base.lf0_delta.var");
	     }
	     ...
	     shell("$MLPG -l $ordr{'lf0'} -d $nndatdir/win/lf0.win2.f -d $nndatdir/win/lf0.win3.f $gendir/$base.lf0_pdf > $gendir/$base.lf0_ip");
	      ...
	      print "No MLPG on lf0\n";
	     # only generate the static compoenent
...
\"$gendir/$base.lf0_ip\", \"$f0gendir/$base.vuv\", \"$gendir/$base.lf0\", $vuThres)'");
	  ...
	  if ($useMLPG){
	     ...
	     for ( $i = 1 ; $i <= $T ; $i++ ) {
		 shell_mute("cat $nndatdir/var/bap.var >> $gendir/$base.bap_delta.var");
	     }
	     ...
	     shell("$MLPG -l $ordr{'bap'} -d $nndatdir/win/bap.win2.f -d $nndatdir/win/bap.win3.f $gendir/$base.bap_pdf > $gendir/$base.bap");
	      ...
	      print "No MLPG on BAP\n";
	     shell("mv $apgendir/$base.bap_delta $apgendir/$base.bap");
	 }
         $apgendir = "$gendir";
      }
      if ($synWav && -s "$apgendir/$base.bap") {
          #bap2ap( "$apgendir/$base.bap", "$gendir/$base.ap" );
	  $cmd = "$BNDAP2AP -s $sr -l $fp $apgendir/$base.bap | $SOPR -d 20 -EXP -f 0.00000005 -c 0.99999995 > $gendir/$base.ap";
	  shell($cmd);
      ...
      if ($synWav && -s "$gendir/$base.sp" && -s "$gendir/$base.f0" && -s "$gendir/$base.ap" ) {
         ...
         if(0){shell("$X2X +fa $gendir/$base.f0 > $gendir/$base.f0.a");
	 ...
	 $cmd = "$WORLD/world_synthesisap -f -p ".(($fs*1000)/$sr)." -l $fp -s $sr -r 30";
	 $cmd = "$cmd $gendir/$base.f0 $gendir/$base.sp $gendir/$base.ap $gendir/$base.wav";
	 print "$cmd\n";
	 shell("$cmd");
      ...
      shell("rm -f $gendir/$base.mgc_delta.var $gendir/$base.mgc_delta $gendir/$base.mgc_pdf");
      shell("rm -f $gendir/$base.f0");
      shell("rm -f $gendir/$base.lf0_delta.var $gendir/$base.lf0_delta $gendir/$base.lf0_pdf");
      shell("rm -f $gendir/$base.ap");
      shell("rm -f $gendir/$base.bap_delta.var $gendir/$base.bap_delta $gendir/$base.bap_pdf");
   }
}
   ...
   foreach $file (@FILE) {
	 ...
	 shell($cmd);
	  ...
	  if(0){
	      ...
	      $line = "$SYNTHESIS_FFT -f $sr -spec -fftl $fp -shift " . ( ( $fs * 1000 ) / $sr ) . " -sigp 16 -cornf 4000 -float ";
	      $line .= "-apfile $gendir/$base.ap $gendir/${base}.f0.a $gendir/$base.sp $gendir/$base.wav";
	      print "$line\n";
	      shell($line);
	      shell("rm -f $gendir/$base.f0.a");}
      ...
      shell("rm -f $gendir/$base.ap");
      shell("rm -f $gendir/$base.bap_delta $gendir/$base.bap_delta.var $gendir/$base.bap_pdf");
   }
}
