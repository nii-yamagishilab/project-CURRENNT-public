...
sub gen_wave($$$$$$) {
   ...
   foreach $file (@FILE) {
      ...
      if ( -s "$spgendir/$base.mgc_delta" ) {
	     ...
	     if ( -s "$gendir/$base.mgc_delta"){
		 shell("mv $gendir/$base.mgc_delta $gendir/$base.mgc");
	     }elsif (-s "$spgendir/$base.mgc"){
		 shell("cp $spgendir/$base.mgc $gendir/$base.mgc");
	     }else{
		 shell("cp $spgendir/$base.mgc_delta $gendir/$base.mgc");
	     }
         }
	 $spgendir = "$gendir";
      }
	  ...
	  if ( $useSigPF && $useSigPF2 > 0.001) {
	      shell( "$MCPF -a $fw -m " . ( $ordr{'mgc'} - 1 ) . " -b $useSigPF2 -l $il $spgendir/$base.mgc > $gendir/$base.p_mgc" );
	      $mgc = "$gendir/$base.p_mgc";
	  }
	  else {
	      print "No post-filtering\n";
	 ...
	 if (-s "$gendir/$base.lf0_ip"){
      ...
      if ( -s "$apgendir/$base.bap_delta" ) {
	     ...
	     if (-s "$gendir/$base.bap_delta"){
		 shell("mv $gendir/$base.bap_delta $gendir/$base.bap");
	     }elsif (-s "$apgendir/$base.bap"){
		 shell("cp $apgendir/$base.bap $gendir/$base.bap");
	     }else{
		 shell("cp $apgendir/$base.bap_delta $gendir/$base.bap");
	     }
	 }
         $apgendir = $gendir;
      }
